...
type: vision
tags: [north-star, vision, federation, departments, zibby-dna]
created: 2026-07-17
status: active
...
The organizing idea already has a face: the **Velín** — eight department orbs on the
...
global scheduler runs everything, a global briefing reports everything. The departments
(Development, Monitoring & Ops, Security, Release Management, Incident Response, R&D, Communications, QA & Architecture) exist as an identity
registry and a derived status view — a UI grouping over the monolith.

The finished system inverts this. **Each department is an accountable domain owner.**
The global layer shrinks to a switchboard, a gate floor, and a vault. The operator
talks to the Velín; the Velín talks to its departments; departments do the work.

A department is real only when **removing it would break behavior, not just a screen.**
...
3. **An autonomy policy.** Each department layers its own gate rules and tier defaults
   above the global floor — harden-only, never weaker. Communications's confidence threshold
   for auto-replies and Incident Response's escalation shape are department policy, not global
   constants.
4. **A memory shelf.** Each department has its own MOC in the vault. Runs it owns are
...
5. **Accountability.** "Co dělá Development?" is answerable at any moment from the record:
   the briefing has a line per department, the activity log is department-tagged, and
   each orb's state on the map is backed by that same data.
6. **A heartbeat duty (watchers only).** Monitoring & Ops ticks channels and monitors. QA & Architecture
   periodically audits the codebase. Security periodically scans dependencies and
   secrets. A watcher department that never wakes on its own is not a watcher.
...
- **Development — Kovárna doručení.** Owns the delivery loop (Architekt → Kodér ⇄ Review →
  Tester → Dokumentátor), worktrees, checkpoints, and PR outputs. _Mostly built._
- **Monitoring & Ops — Tep systému.** Owns every inbound heartbeat: channel polling, calendar,
  CI/CD monitors, and the monitor seam (Sentry next). _Mostly built, needs ownership._
- **Communications — Hlas navenek.** Owns triage of inbound conversation and every outbound
...
- **Security — Strážce hranic.** Owns security posture: dependency CVE audits, secret
  leak scans on outbound artifacts, permission drift. _Empty chair today._
- **QA & Architecture — Tkadlec kvality.** Owns proactive code quality: scheduled architecture and
  dead-code audits (graphify, madge, knip), findings filed as proposals to Development.
  _Empty chair today — but the tooling already exists in the repo._

**New chairs (seated by this vision):**

- **Knowledge Management — Paměť rodu.** Owns the second brain: the vault, grounding, run recording,
...
- **Finance — Správce pokladny.** Owns budget caps, usage windows, token spend,
  limit-pause/resume governance, and the spend-past-cap gate. The federation's
  economist: it knows what every department costs and says so in the briefing.
  _The machinery is built; the chair is new._
- **Personal Office — Krb domova** _(seated later, with the personal domain)_. Owns the
...
- **Decide** — Tier-3 approvals, curated by Incident Response so every decision is worth making.
- **Review & Merge** — the one irreversible click, prepared by Development, staged by
  Release Management. Opening a PR is Tier-2 everywhere — one posture, no exceptions per runner;
...
The federation may only widen its autonomy with evidence. Communications does not start
auto-replying because a flag flipped; it starts when its ledger shows its drafted
replies were approved unedited N times in a row. Monitoring & Ops's adapters are trusted because
...
> **Ownership is explicit.** Every unit of work belongs to exactly one department, and
> every department can account for everything it owns. No orphan agents, no derived
