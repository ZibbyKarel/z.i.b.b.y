...
Spec: `docs/superpowers/specs/2026-07-31-design-match-design.md`. CLI: `tools/design-match/cli.mjs` (v1.2.0).
...
Inventura regionů (1440×900):
  [1] .epic-card                  360×140   @ (24,96)   ▸ r1.png
  [2] .board > .column                480×640  @ (400,80)  ▸ r2.png
Vybrán region [1]: .epic-card — pokud je špatně, spusť znovu s --region <n>.
```

Open the crop (`r1.png`, …), and if `[1]` is the wrong region, rerun with
`--region 2` (1-based, validated against however many candidates were found).
`measure` writes `design.png` (the cropped shot) and `spec.json` for whichever
region wins.
...
| `--strict-wrappers` | off                                        | see `references/skeleton-rules.md` — must be the same on the matching `compare`, or the two trees collapse differently           |
...
| `--strict-wrappers` | off                                            | must match whatever `measure` used for this slug                                 |
...
- a hard ceiling of 5 rounds — the next attempt is refused with a park;
...
| 3    | `CHYBA`    | `compare`/`measure` itself failed — a bad invocation, a missing `spec.json`, a browser that wouldn't launch, a failed write | fix the invocation/environment first — reading this as "continue" loops forever against a dead tool |
...
| File                  | Read it when                                                                                                                                                     |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `report.md`           | first, always — verdict, round history, masked regions, and a list of every other file this run wrote                                                            |
| `skeleton.md`         | the gate is red — this names the structural difference                                                                                                           |
| `values.md`           | skeleton is green and a value delta needs tuning — same address space as `skeleton.md`: one DOM walk produces both, so a path names the same node in either file |
| `tokens.md`           | reviewing DS token growth before approving a new one                                                                                                             |
| `components.md`       | justifying why a new component was created instead of reusing one — currently an empty stub the tool never auto-populates (see note below)                       |
...
| `app.png`             | the app screenshot from the most recent `compare` round                                                                                                          |
| `r1.png`, `r2.png`, … | the numbered region preview crops `measure` printed, for picking `--region <n>` with the image in hand                                                           |

`components.md`'s content is always the literal `componentDecisions: []`
payload today — nothing in the shipped tool populates it. Recording _why_ a
new component was justified (which existing DS candidates were tried, why
each was rejected) is currently a manual step for whoever drives the loop, not
