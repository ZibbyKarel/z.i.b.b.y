...
| **page** the browser refuses to photograph (region previews)       | a different refusal from the row above, and `measure`-only. What the browser refused is the whole **document**, so no candidate got a crop and the inventory never printed: `design-match: náhledy regionů se nepodařilo vyfotit — prohlížeč odmítl snímek celé stránky o rozměrech 20000×9000 px. Dokument mockupu je nad limitem snímkování v prohlížeči, takže z tohoto běhu nevznikl žádný náhled a inventura regionů se nevypsala. Zmenši stránku mockupu; bez snímku stránky se region nedá vybrat podle náhledu.` The per-slug directory is genuinely empty here, and there is no `spec.json`. Deliberately **not** "pick a different `--region`" — with no inventory printed, that is a remedy the run has not put you in a position to take |
