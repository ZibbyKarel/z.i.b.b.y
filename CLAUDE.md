...
## Design system — zdroj UI primitiv

DS (`libs/design-system`) je **výchozí zdroj UI primitiv** pro všechny generované komponenty.
Appka skládá UI z DS, netvoří vlastní primitiva.

- Když potřebné primitivum **neexistuje**, explicitně rozhodni: přidat ho do DS, nebo nechat
  UI lokální v appce (doménový kompozit). Rozhodnutí nenechávej implicitní.
- Každá DS komponenta je **zdokumentovaná** (story) a **ověřená** (test) — viz Testy + Storybook.
- Sdílená UI rozhodnutí drž **centralizovaná** v DS, ne rozkopírovaná po appce.
...
Ikony drž pod `libs/design-system/src/components/assets/icons`. Komponenta `Icon` renderuje glyf přes
`name` prop (union `IconName`, sada glyphů v `iconNames`).

- **Žádné per-icon testy** ani **per-icon stories.**
- Místo toho jedna **sdílená story**, která zobrazí celou sadu ikon najednou.
...
## Dashboard chrome (v DS, router-agnostic + doménově neutrální)

`DashboardShell`, `Sidebar`, `TopBar`, `ContextSwitch` zůstávají v DS a **neimportují
doménové typy**.
...
- `ContextName` (`"home"|"work"`) je téma-koncept DS — chrome ho bere z
  `DesignSystemContext/contextTokens`, ne z `domain.ts`.
- `NavItem` je chrome typ (žije v `Sidebar.tsx`, re-export z indexu), ne doménová entita.
- Doménový wallet/limity se injektuje **slotem**: `TopBar`/`DashboardShell` mají
  `walletSlot?: ReactNode`; appka předá svůj `LimitsWidget`
  (`apps/web/features/dashboard/components/LimitsWidget.tsx`).
...
žijí v `apps/web/features/<doména>/components/`. Patří sem mj. `AgentCard`, `AgentRow`,
`ApprovalCard`, `ActivityFeed`, `IntegrationCard`, `SkillTile`, `RunModal`, `PipelineCard`,
`PipelineRunModal`, `PhaseChain` (+ `ModelBadge`/`ThinkBadge`) a `LimitsWidget`.
...
**Každá DS komponenta má test.** Výjimka: ikony nemají per-icon testy.
...
**Každá DS komponenta má story** (Overview + Playground). Výjimka: ikony nemají per-icon stories —
jen jedna sdílená story s celou sadou.
...
- Tvořit vlastní UI primitiva v appce, když je lze přidat do DS (nejdřív explicitně rozhodni)
- Přidávat per-icon testy nebo per-icon stories
