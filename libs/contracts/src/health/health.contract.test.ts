...
describe("health schema", () => {
  ...
  it("rejects an unknown subsystem name or status", () => {
    const base = { status: "ok", uptime: 1, timestamp: new Date().toISOString(), claude: { ok: true } }
    expect(HealthSchema.safeParse({ ...base, subsystems: [{ name: "db", status: "ok" }] }).success).toBe(false)
    expect(HealthSchema.safeParse({ ...base, subsystems: [{ name: "vault", status: "weird" }] }).success).toBe(false)
  })
