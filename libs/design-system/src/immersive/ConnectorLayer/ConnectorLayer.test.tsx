...
const nodes: ConnectorNode[] = [
  { id: "knw", x: 40, y: 40, color: "#8b5cf6", live: true },
  { id: "atlas", x: 260, y: 40, color: "#22d3ee", live: false },
  { id: "dev", x: 150, y: 220, color: "#f97316", live: false },
];

describe("ConnectorLayer", () => {
  it("renders a decorative root svg", () => {
    render(<ConnectorLayer center={{ x: 150, y: 130 }} nodes={nodes} />);
    expect(screen.getByTestId(ConnectorLayerTestId.Root)).toHaveAttribute("aria-hidden", "true");
  });

  it("renders one connector group per node", () => {
    render(<ConnectorLayer center={{ x: 150, y: 130 }} nodes={nodes} />);
    for (const n of nodes) {
      expect(screen.getByTestId(`${ConnectorLayerTestId.Connector}-${n.id}`)).toBeInTheDocument();
    }
  });
  ...
  it("adds a live dashed overlay on top of the base stroke for a live node", () => {
    render(<ConnectorLayer center={{ x: 150, y: 130 }} nodes={nodes} />);
    const group = screen.getByTestId(`${ConnectorLayerTestId.Connector}-knw`);
