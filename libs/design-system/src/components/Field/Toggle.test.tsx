...
describe("Toggle", () => {
  ...
  it("shows a hint linked to the switch", () => {
    render(<Toggle checked hint="Spustí se po uložení" label="Auto-run" onChange={vi.fn()} />)
    const hint = screen.getByTestId(FieldTestId.Hint)
    expect(hint).toHaveTextContent("Spustí se po uložení")
    expect(screen.getByTestId(ToggleTestId.Control)).toHaveAttribute("aria-describedby", hint.id)
  })
})
