  ...
  return (
    <div onClick={() => onSelect(a.id)} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{
        ...
        borderRadius: Z.rCtl, padding: '13px 14px', cursor: 'pointer', transition: 'all .14s',
        boxShadow: selected ? `0 8px 24px rgba(0,0,0,0.32)` : 'none', opacity: dec ? 0.6 : 1,
      }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 32, height: 32, flex: '0 0 auto', borderRadius: Z.rCtl, display: 'grid', placeItems: 'center', background: `${r.c}18`, color: r.c, border: `1px solid ${r.c}44` }}>
...
const ApprovalDetail = ({ a, accent, decided, onDecide }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      {/* header */}
      <HudPanel accent={r.c} live={!dec} pad={20} style={dec ? {} : { borderColor: `${r.c}55` }}>
      ...
      <HudPanel accent={r.c} pad={18}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '11px 13px', background: `${r.c}10`, border: `1px solid ${r.c}33`, borderRadius: Z.rCtl }}>
        ...
        {dec ? (
          <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 11, padding: '13px 15px', borderRadius: Z.rCtl, background: dec === 'ok' ? 'rgba(63,207,142,0.1)' : 'rgba(154,167,180,0.08)', border: `1px solid ${dec === 'ok' ? Z.ok + '44' : Z.line}` }}>
            ...
            <button onClick={() => onDecide(a.id, null)} style={{ marginLeft: 'auto', fontFamily: Z.mono, fontSize: 11, padding: '7px 12px', cursor: 'pointer', borderRadius: Z.rCtl, color: Z.inkDim, background: 'transparent', border: `1px solid ${Z.line}` }}>Vrátit do fronty</button>
          </div>
        ) : (
            ...
            {byRisk.map(([rk, n]) => {
              ...
              return (
                <div key={rk} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 30, height: 30, borderRadius: Z.rCtl, display: 'grid', placeItems: 'center', background: `${r.c}16`, color: r.c, border: `1px solid ${r.c}40` }}><Icon name={r.glyph} size={15} /></div>
