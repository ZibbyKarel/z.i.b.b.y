...
// Bar — matný (světlo jen na živém; bary už nesvítí). `glow` ponechán pro kompatibilitu, ignoruje se.
const Bar = ({ pct, color, h = 6, glow = false, track = 'rgba(255,255,255,0.07)' }) =>
...
// Dot — svítí a pulzuje JEN když je živý (pulse). Klidové tečky jsou matné.
const Dot = ({ color, pulse = false, size = 8 }) =>
<span style={{ position: 'relative', width: size, height: size, flex: '0 0 auto', display: 'inline-block' }}>
    {pulse && <span style={{ position: 'absolute', inset: -3, borderRadius: '50%', background: color, opacity: 0.3, animation: 'zpulse 2s ease-out infinite' }} />}
    <span style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: color, boxShadow: pulse ? `0 0 7px ${color}aa` : 'none' }} />
  </span>;
...
const RunBtn = ({ accent, label = 'Spustit', size = 'md', onClick, icon = 'play' }) => {
  ...
  return (
    ...
    style={{
      ...
      border: `1px solid ${accent}`, borderRadius: Z.rCtl,
      transition: 'all .16s'
    }}>
...
const GhostBtn = ({ children, icon, onClick, accent = Z.inkDim }) => {
  ...
  return (
    ...
    style={{
      ...
      border: `1px solid ${h ? Z.lineHi : Z.line}`, borderRadius: Z.rCtl, transition: 'all .14s'
    }}>
...
<div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
    <div style={{ fontFamily: Z.mono, fontSize: 11, letterSpacing: '0.14em', color: Z.inkFaint, textTransform: 'uppercase' }}>{children}</div>
    {right}
  </div>;
  ...
  display: 'flex', flexDirection: 'column', padding: '22px 14px'
    ...
    <div style={{ padding: '4px 6px 22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <img alt="ZIBBY" src="uploads/icon.png" style={{ width: 44, height: 44, objectFit: 'contain' }} />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      return (
        <div key={n.id} onClick={() => onNav && onNav(n.id)} style={{
          display: 'flex', alignItems: 'center', gap: 11, padding: '9px 11px', borderRadius: Z.rCtl, cursor: 'pointer',
          ...
          position: 'relative', fontSize: 13.5, fontWeight: on ? 600 : 500
            ...
            {n.badge && <span style={{ fontFamily: Z.mono, fontSize: 11, fontWeight: 600, color: n.alert ? Z.warn : Z.inkFaint }}>{n.badge}</span>}
          </div>);
    ...
    <div style={{ marginTop: 'auto', paddingTop: 12, borderTop: `1px solid ${Z.line}` }}>
      ...
      display: 'flex', alignItems: 'center', gap: 11, padding: '9px 11px', borderRadius: Z.rCtl, cursor: 'pointer',
      color: active === 'settings' ? Z.ink : Z.inkDim, background: active === 'settings' ? Z.workDim : 'transparent',
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
      <div style={{ position: 'relative', width: 30, height: 30, flex: '0 0 auto' }}>
        <svg height="30" style={{ transform: 'rotate(-90deg)' }} viewBox="0 0 30 30" width="30">
          <circle cx="15" cy="15" fill="none" r={R} stroke="rgba(255,255,255,0.09)" strokeWidth="3" />
          <circle cx="15" cy="15" fill="none" r={R} stroke={c} strokeDasharray={C} strokeDashoffset={off}
            strokeLinecap="round" strokeWidth="3" style={{ transition: 'stroke-dashoffset .4s' }} />
        </svg>
...
const LimitsTopBar = () => {
  ...
  return (
    <div onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} style={{ position: 'relative' }}>
      ...
      }} title="Interaktivní limity · Claude">
      ...
      {open && (
        <div style={{
          position: 'absolute', top: 'calc(100% + 10px)', right: 0, width: 300, zIndex: 60,
          background: Z.panelHi, border: `1px solid ${Z.lineHi}`, borderRadius: Z.rPanel, padding: 16,
          boxShadow: '0 18px 50px rgba(0,0,0,0.5)',
        }}>
          <Mono style={{ fontSize: 10, letterSpacing: '0.14em', color: Z.inkFaint, textTransform: 'uppercase' }}>Interaktivní limity · Claude</Mono>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 15, marginTop: 13 }}>
            <LimitRow d={r} />
            <LimitRow d={w} />
          </div>
          <Mono style={{ fontSize: 9.5, color: Z.inkFaint, display: 'block', marginTop: 11 }}>čerpá tvůj chat · nezávislé na agentech</Mono>
        </div>
      )}
    </div>
  );
...
const TopBar = ({ accent, nav = 'overview', lang = 'cs', onLang, onVoice, onNewTask }) =>
<header style={{
  ...
  borderBottom: `1px solid ${Z.line}`, background: Z.bg1, position: 'relative', zIndex: 20
    ...
    {/* command / search bar — ve flow (ne absolutn\u00ed), aby nekolidoval s TASK / ovl\u00e1d\u00e1n\u00edm */}
    <button style={{
      display: 'flex', alignItems: 'center', gap: 10, flex: '0 1 360px', minWidth: 150,
      marginLeft: 'auto', marginRight: 'auto', padding: '9px 14px',
      background: Z.bg0, border: `1px solid ${Z.line}`, borderRadius: Z.rCtl, color: Z.inkFaint, cursor: 'pointer'
    }} title="Příkaz nebo skill (⌘K)">
      <Icon name="search" size={14} />
      <span style={{ fontSize: 12.5, color: Z.inkFaint, flex: 1, textAlign: 'left' }}>Příkaz nebo skill…</span>
      <span style={{ fontFamily: Z.mono, fontSize: 10, color: Z.inkFaint, border: `1px solid ${Z.line}`, borderRadius: 4, padding: '1px 6px' }}>⌘K</span>
    </button>
    {onNewTask && (
      <NewTaskBtn
        accent={accent}
...
Object.assign(window, { Bar, Dot, Mono, RunBtn, GhostBtn, SectionLabel, Sidebar, LimitsWidget, TopBar, LangSwitch, limitColor, MiniBar, Sparkline, VoiceToggleBtn, LimitsTopBar, LimitRing });