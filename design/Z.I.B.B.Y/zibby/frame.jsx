...
// Frame: full velín screen. Atmosféru dříve nesly scanlines + mřížka — audit je ruší;
// hloubku teď drží radiální gradient pozadí. `skin` ponechán pro kompatibilitu (ignoruje se).
const Frame = ({ children, skin = 'velin' }) => {
  return (
    <div style={{
      position: 'relative', width: '100%', height: '100%',
      background: `radial-gradient(ellipse 120% 90% at 50% -8%, #101722 0%, ${Z.bg1} 58%)`,
      fontFamily: Z.sans, color: Z.ink, overflow: 'hidden'
    }}>
...
// Generic card surface — sjednocený povrch (radius 10, bez glow ve výchozím stavu)
const Card = ({ children, accent, glow = false, hud = false, pad = 18, style }) => (
  <div style={{
    position: 'relative', background: Z.panel, border: `1px solid ${Z.line}`, borderRadius: Z.rPanel,
    padding: pad, ...style,
  }}>
    {children}
  </div>
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        width: 540, maxWidth: '100%', background: Z.panelHi, border: `1px solid ${Z.lineHi}`, borderRadius: Z.rPanel,
        ...
        {!launched ? (
          <div style={{ padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginTop: 8 }}>
              {projectList.map((p) => (
                <button key={p} onClick={() => setProj(p)} style={{
                  fontFamily: Z.mono, fontSize: 11, padding: '6px 11px', cursor: 'pointer', borderRadius: Z.rCtl,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 16, padding: '9px 12px', background: Z.bg0, border: `1px solid ${Z.line}`, borderRadius: Z.rCtl }}>
...
// HUD panel — sjednocený povrch (radius 10). Rohové závorky JEN když `live`
// (podpis znovu nese význam = živý panel). Titulek = T.label (bez `//`).
function HudPanel({ children, accent, title, right, style, pad = 18, live = false }) {
  return (
    <div style={{ position: 'relative', background: Z.panel, border: `1px solid ${live ? (accent || Z.run) + '3d' : Z.line}`, borderRadius: Z.rPanel, padding: pad, ...style }}>
      {live && <Corners color={accent || Z.run} inset={6} />}
      {title && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
          <Mono style={{ fontSize: 11, letterSpacing: '0.14em', color: Z.inkDim, textTransform: 'uppercase', fontWeight: 500 }}>{title}</Mono>
          {right}
        </div>
      )}
      {children}
