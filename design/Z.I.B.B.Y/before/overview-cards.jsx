  ...
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{
        ...
        borderRadius: hud ? 2 : 11, padding: 15, transition: 'all .15s', cursor: 'default',
        boxShadow: h ? `0 6px 22px rgba(0,0,0,0.35)` : 'none',
      }}>
      {hud && <Corners color={accent} inset={5} />}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11 }}>
        <div style={{ width: 34, height: 34, flex: '0 0 auto', borderRadius: hud ? 2 : 8, display: 'grid', placeItems: 'center', background: accentDimOf(skill.ctx), color: accent, border: `1px solid ${accent}33` }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: `1px solid ${Z.line}` }}>
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 7 }}>
          <div style={{ flex: 1 }}><Bar glow color={accent} h={4} pct={a.pct} /></div>
          <Mono style={{ fontSize: 10, color: accent, fontWeight: 600 }}>{a.pct}%</Mono>
        </div>
      </div>
