...
import { Chip, Markdown, Panel, Stack, Typography } from "@zibby/design-system";
 ...
 * same note is also visible as an ordinary vault note. Moved from Settings →
 * Self-Knowledge into Knowledge → Distill's "Self-model" section (ZB-09,
 * ROUTE-MAP §3) — its i18n strings stay under the `settings` namespace
 * (`settings.selfKnowledge.*`) since only its location moved, not its copy.
 */
export function SelfKnowledgeSection() {
  ...
  if (query.isPending) {
    return (
      <Panel header={t("selfKnowledge.title")} padding="300">
        <QueryLoading />
      </Panel>
    );
  }
