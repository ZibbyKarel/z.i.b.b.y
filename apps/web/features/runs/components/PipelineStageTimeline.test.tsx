...
function pipelineRun(stageRuns: RunView["stageRuns"]): RunView {
  return {
    runId: "delivery_1",
    kind: "pipeline",
    owner: "delivery",
...
describe("PipelineStageTimeline (28)", () => {
  ...
  it("renders one row per stage with its phase + retried attempt", () => {
    render(<PipelineStageTimeline run={pipelineRun(stages)} />);
  ...
  it("shows an empty-state (+ definition link) when the run has no stages yet", () => {
    render(<PipelineStageTimeline run={pipelineRun([])} />);
