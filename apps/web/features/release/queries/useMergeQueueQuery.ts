...
export function getMergeQueueQueryKey(projectId?: string) {
  return ["release-queue", projectId ?? "all"] as const;
}
...
/**
 * Release's cross-project merge queue (`GET /api/release/queue`, NS2 F5b/F7b-1) —
...
export function useMergeQueueQuery(projectId?: string) {
  return apiClient.rel.getMergeQueue.useQuery({
