  ...
  it("GET /api/briefing surfaces the pending approval and the watched channel", async () => {
    ...
    await until(async () => {
      const pending = await request(app.getHttpServer())
        ...
        .expect(200);
      return (pending.body as Array<{ runId: string }>).some((a) => a.runId === runId)
        ? true
        : null;
    });
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        text: "Tady je nabídka a smlouva s deadline",
        receivedAt: "2026-06-12T00:00:00.000Z",
      }),
    );
    await app.get(ChannelWatcherService).tick();

    const briefing = (await request(app.getHttpServer()).get("/api/briefing").expect(200))
      .body as Briefing;
