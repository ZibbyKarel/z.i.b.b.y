...
describe("Skill categories API (e2e)", () => {
  ...
  it("creates a category and refuses to delete it while a skill uses it (409)", async () => {
    await request(app.getHttpServer()).post(BASE).send({ name: "Média", glyph: "film" }).expect(201)
    await request(app.getHttpServer())
      .post("/api/skills")
      .send({ id: "tmdb-renamer", category: "Média", instructions: "Rename media." })
      .expect(201)

    await request(app.getHttpServer()).delete(`${BASE}/${encodeURIComponent("Média")}`).expect(409)

    await request(app.getHttpServer()).delete("/api/skills/tmdb-renamer").expect(200)
    await request(app.getHttpServer()).delete(`${BASE}/${encodeURIComponent("Média")}`).expect(200)
  })
})
