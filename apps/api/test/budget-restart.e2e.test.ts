...
describe("Budget restart (e2e)", () => {
  ...
  const taskById = async (id: string) => {
    ...
    return (res.body as Array<{ id: string; status: string; approvalId?: string }>).find((t) => t.id === id)
  }
  ...
  it("a held task survives a reboot and is still resumable via its approval", async () => {
    ...
    const pending = (await request(server()).get("/api/approvals?status=pending")).body as Array<{ id: string }>
    ...
    const dispatched = await poll(() => taskById(heldId), (t) => t?.status === "dispatched")
  ...
  it("a queued task drains on the bootstrap sweep after its blocking run is gone", async () => {
    ...
    const queued = await request(server()).post("/api/tasks").send({ text: "beta queued behind blocker" })
    ...
    await reboot()
    const after = await poll(() => taskById(queuedId), (t) => t?.status === "dispatched")
