...
function grantDirs() {
  ...
  for (let i = 0; i < argv.length; i++)
    if (argv[i] === "--add-dir" && argv[i + 1]) dirs.push(argv[i + 1]);
  return dirs;
}
...
async function main() {
  ...
  if (process.env.FAKE_CLAUDE_PRODUCE) {
    await fs.writeFile(
      ...
      process.env.FAKE_CLAUDE_PRODUCE_BODY ??
  ...
  if (process.env.FAKE_CLAUDE_INTENT) {
    ...
    if (process.env.FAKE_CLAUDE_EXEC_CMD) {
      ...
      execFileSync("/bin/sh", ["-c", process.env.FAKE_CLAUDE_EXEC_CMD], {
        cwd,
        env,
        stdio: "ignore",
      });
  ...
  if (process.env.FAKE_CLAUDE_DELETE) {
    ...
    const targets = process.env.FAKE_CLAUDE_DELETE.split(",")
      .map((s) => s.trim())
      .filter(Boolean);
