...
describe("Categories API (e2e)", () => {
  ...
  it("creates a category, then lists and rejects a duplicate", async () => {
    const created = await request(app.getHttpServer()).post(BASE).send({ name: "Finance", glyph: "dollar" })
    ...
    const dup = await request(app.getHttpServer()).post(BASE).send({ name: "Finance", glyph: "cart" })
