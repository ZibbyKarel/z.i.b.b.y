...
describe("Activity log (e2e)", () => {
  ...
  const ofKind = (entries: ActivityEntry[], kind: ActivityKind) =>
  ...
  it("records gate-decision + approval-requested + approval-approved for a gated run", async () => {
    ...
    const approval = await until(async () => {
      const res = await request(app.getHttpServer())
        ...
        .expect(200);
      return (
        (res.body as Array<{ id: string; runId: string }>).find((a) => a.runId === runId) ?? null
      );
    });
    ...
    const entries = await until(async () => {
      const all = await activity();
      return all.some((e) => e.kind === "approval-approved" && e.refs.approvalId === approval.id)
        ? all
        : null;
    });

    const gate = ofKind(entries, "gate-decision").find((e) => e.refs.action === "payment");
    const requested = ofKind(entries, "approval-requested").find(
      (e) => e.refs.approvalId === approval.id,
    );
    const approved = ofKind(entries, "approval-approved").find(
      (e) => e.refs.approvalId === approval.id,
    );
