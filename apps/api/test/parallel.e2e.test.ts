...
describe("Parallel engagements (e2e)", () => {
  ...
  const taskById = async (id: string) => {
    ...
    return (res.body as Array<{ id: string; status: string; projectId?: string }>).find((t) => t.id === id)
  }

  const budgetRow = async (projectId: string) => {
    const res = await request(server()).get("/api/budget")
    return (res.body.projects as Array<{ projectId: string; queued: number; running: number; daily: { used: number } }>).find(
      (p) => p.projectId === projectId,
    )
  }

  beforeAll(async () => {
    for (const key of ["agents", "pipelines", "runs", "tasks", "projects", "approvals", "ledger", "activity", "pa", "pb"]) {
  ...
  it("queues A's second task at maxConcurrent, runs B immediately, and drains the queue on terminal", async () => {
    ...
    const drained = await poll(() => taskById(a2Id), (t) => t?.status === "dispatched")
    expect(drained?.status).toBe("dispatched")

    // The ledger attributed both A runs and the B run to the right engagement.
    const alpha = await poll(() => budgetRow("alpha"), (r) => (r?.daily.used ?? 0) >= 2)
