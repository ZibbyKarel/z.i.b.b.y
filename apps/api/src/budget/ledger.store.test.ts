...
describe("BudgetLedgerStore", () => {
  ...
  it("counts only the matching project on a given day", async () => {
    ...
    await store.record({ at: now.toISOString(), projectId: "alpha", runRef: "r1", kind: "agent" }, now)
    await store.record({ at: now.toISOString(), projectId: "alpha", runRef: "r2", kind: "agent" }, now)
    await store.record({ at: now.toISOString(), projectId: "beta", runRef: "r3", kind: "agent" }, now)
  ...
  it("a record from another day does not count toward today", async () => {
    ...
    await store.record({ at: yesterday.toISOString(), projectId: "alpha", runRef: "r1", kind: "agent" }, yesterday)
  ...
  it("throws LedgerUnreadableError when the dir is a file (fail-closed signal)", async () => {
    ...
    await expect(broken.countDaily("alpha", new Date("2026-06-12T08:00:00.000Z"))).rejects.toThrow()
  })
})
