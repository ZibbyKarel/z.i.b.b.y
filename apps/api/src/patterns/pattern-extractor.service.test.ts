...
function makeService(entries: ActivityEntry[] = [], vaultBody = "") {
  ...
  const vault = {
    note: vi.fn(async () => {
      ...
      return { id: "patterns/suggestions", title: "Pattern Suggestions", body: vaultBody, tier: "memory" }
    }),
...
describe("PatternExtractorService", () => {
  ...
  it("ignores entries that don't meet the 3-occurrence threshold", async () => {
    const entries = [makeEntry("approval-approved", "deploy", 1), makeEntry("approval-approved", "deploy", 2)]
  ...
  it("writes proposals to vault when patterns found", async () => {
    const entries = Array.from({ length: 3 }, (_, i) => makeEntry("approval-approved", "send-email", i + 1))
