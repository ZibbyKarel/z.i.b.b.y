...
function build(over: {
  ...
  const vault = {
    index: vi.fn(async () => (over.vaultThrows ? Promise.reject(new Error("ENOENT")) : [])),
  };
  const integrations = {
...
describe("SubsystemHealthService", () => {
  ...
  it("treats a disabled tick loop (tickMs<=0) as ok, not degraded", async () => {
    const rows = await build({
      scheduler: { running: false, tickMs: 0, lastTickAt: null },
    }).probeAll();
  ...
  it("degrades the scheduler when configured to run but not armed", async () => {
    const rows = await build({
      scheduler: { running: false, tickMs: 60000, lastTickAt: null },
    }).probeAll();
