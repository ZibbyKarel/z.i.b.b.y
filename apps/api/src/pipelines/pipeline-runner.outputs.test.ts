...
async function makeService(dir: string, pipeline: Pipeline): Promise<{ service: PipelineRunnerService; d: Doubles }> {
...
async function seedRun(
  ...
  const run: PipelineRun = {
    ...
    cwd,
    ...(workspacePath
      ? { workspace: { branch: "zibby/x", path: workspacePath, baseRef: "HEAD" }, projectPath: workspacePath }
      : {}),
  }
...
function runOutputs(service: PipelineRunnerService, run: PipelineRun, pipeline: Pipeline): Promise<void> {
  return (
    service as unknown as {
      runOutputs(r: PipelineRun, p: Pipeline, from: number, ids: string[]): Promise<void>
    }
  ).runOutputs(run, pipeline, 0, pipeline.phases.map((p) => p.id))
}
...
describe("PipelineRunnerService — output sinks", () => {
  ...
  it("pr sink resume — approved: runs the gated push with the parsed title, run done", async () => {
    ...
    expect(await fs.readFile(path.join(run.cwd, "pr-draft.md"), "utf8")).toBe("# Add feature X\n\nDetails.")
    expect(run.status).toBe("done")
