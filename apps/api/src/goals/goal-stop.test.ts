...
describe("decideStop", () => {
  it("parks on budget regardless of everything else", () => {
    expect(decideStop({ satisfied: true, index: 0, maxIterations: 5, budgetOk: false })).toBe("park-budget")
  })

  it("finishes when the verifier is satisfied", () => {
    expect(decideStop({ satisfied: true, index: 0, maxIterations: 5, budgetOk: true })).toBe("satisfied")
  })

  it("continues when unsatisfied and attempts remain", () => {
    expect(decideStop({ satisfied: false, index: 0, maxIterations: 3, budgetOk: true })).toBe("continue")
    expect(decideStop({ satisfied: false, index: 1, maxIterations: 3, budgetOk: true })).toBe("continue")
  })

  it("parks on iterations when the last attempt failed verification", () => {
    expect(decideStop({ satisfied: false, index: 2, maxIterations: 3, budgetOk: true })).toBe("park-iterations")
    expect(decideStop({ satisfied: false, index: 0, maxIterations: 1, budgetOk: true })).toBe("park-iterations")
  })
})
