    ...
    return tsRestHandler(memoryContract, {
      ...
      createNote: async ({ body }) => {
        try {
          ...
          if (error instanceof DuplicateNoteError) return { status: 409, body: { message: error.message } }
          if (error instanceof InvalidNoteIdError) return { status: 422, body: { message: error.message } }
          throw error
        }
      },
      ...
      updateNote: async ({ params: { id }, body }) => {
        try {
          ...
          if (error instanceof NoteNotFoundError) return { status: 404, body: { message: error.message } }
          if (error instanceof InvalidNoteIdError) return { status: 422, body: { message: error.message } }
          throw error
        }
      },
