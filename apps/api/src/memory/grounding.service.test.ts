...
describe("GroundingService.compose", () => {
  ...
  const seedFull = async (vault: VaultService): Promise<void> => {
    await vault.createNote({ id: "north-star", tier: "memory", title: "North Star", body: "The mission." })
    ...
    await vault.createNote({ id: "billing-moc", tier: "knowledge", title: "Billing", body: "Invoices." })
    await vault.createNote({ id: "noise", tier: "knowledge", title: "Noise", body: "Irrelevant." })
    await vault.createNote({ id: "acme", tier: "knowledge", title: "ACME Project", body: "The project note." })
  }
  ...
  it("caps MOCs at two beyond the North Star + project note", async () => {
    ...
    const block = await made.grounding.compose({ task: "x", matchedTerms: ["moc", "rohlik", "billing"] })
