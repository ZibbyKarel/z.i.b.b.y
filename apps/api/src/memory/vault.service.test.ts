...
import { DuplicateNoteError, InvalidNoteIdError, NoteNotFoundError, VaultService } from "./vault.service"
...
describe("VaultService write paths", () => {
  ...
  it("rejects a duplicate id across tiers (409)", async () => {
    ...
    await expect(vault.createNote({ id: "dup", tier: "knowledge", body: "second" })).rejects.toBeInstanceOf(
      DuplicateNoteError,
    )
  })
  ...
  it("updates: missing note throws NoteNotFoundError (404)", async () => {
    await expect(vault.updateNote("ghost", { body: "x" })).rejects.toBeInstanceOf(NoteNotFoundError)
  })
