...
export class GitHubChannelAdapter implements ChannelAdapter {
  ...
  async poll(integration: Integration, creds: CredentialsInput, cursor: string | undefined): Promise<PollResult> {
    ...
    const params = new URLSearchParams({ state: "open", sort: "updated", direction: "asc", per_page: "50" })
    if (cursor) params.set("since", cursor)
    const res = await this.fetchImpl(`${GITHUB_API}/repos/${repo}/issues?${params}`, { headers: this.headers(creds) })
  ...
  async send(integration: Integration, creds: CredentialsInput, item: ChannelItem, text: string): Promise<void> {
    ...
    const res = await this.fetchImpl(`${GITHUB_API}/repos/${integration.config.repo}/issues/${number}/comments`, {
      method: "POST",
