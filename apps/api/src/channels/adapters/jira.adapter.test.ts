...
describe("JiraChannelAdapter", () => {
  it("normalizes issues, derives a jira-<KEY> id, and advances the cursor", async () => {
    const fetchImpl = jsonFetch({
      issues: [
        { key: "BUG-1", fields: { summary: "Login crashes", updated: "2026-06-17T09:00:00.000Z", reporter: { displayName: "Dana" } } },
        { key: "BUG-2", fields: { summary: "Slow search", updated: "2026-06-17T10:00:00.000Z", reporter: { displayName: "Eli" } } },
      ],
    })
  ...
  it("test maps /myself to a TestResult", async () => {
    ...
    expect(await ok.test(jira, { token: "tok" })).toEqual({ ok: true, detail: "authenticated as Dana" })
  ...
  it("send posts a comment to the issue", async () => {
    ...
    const adapter = new JiraChannelAdapter(fetchImpl)
    const item = {
      id: "jira-BUG-1", integrationId: "acme-jira", kind: "jira" as const,
      externalRef: { channel: "BUG", messageId: "BUG-1" }, receivedAt: "2026-06-17T00:00:00.000Z",
