...
export class CalendarChannelAdapter implements ChannelAdapter {
  ...
  private async accessToken(sa: ServiceAccount): Promise<string> {
    ...
    if (!res.ok || !body.access_token) {
      throw new Error(`google token exchange: ${body.error_description ?? body.error ?? `HTTP ${res.status}`}`)
    }
    return body.access_token
  }

  async test(integration: Integration, creds: CredentialsInput): Promise<TestResult> {
    if (integration.config.kind !== "calendar") return { ok: false, detail: "not a calendar integration" }
  ...
  async poll(integration: Integration, creds: CredentialsInput, cursor: string | undefined): Promise<PollResult> {
    ...
    do {
      ...
      if (res.status === 429) throw new Error(`google calendar rate limited (retry_after ${res.headers.get("retry-after") ?? "?"})`)
      const body = (await res.json()) as EventsResponse
      if (!res.ok) throw new Error(`google calendar events: ${body.error?.message ?? `HTTP ${res.status}`}`)
      ...
      if (pageToken && page >= MAX_PAGES) {
        ...
        throw new Error(`google calendar: look-ahead window exceeds ${MAX_PAGES} pages; narrow lookaheadDays`)
      }
    } while (pageToken)
