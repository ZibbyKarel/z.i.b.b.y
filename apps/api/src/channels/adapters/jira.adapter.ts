...
export class JiraChannelAdapter implements ChannelAdapter {
  ...
  async test(integration: Integration, creds: CredentialsInput): Promise<TestResult> {
    ...
    try {
      const res = await this.fetchImpl(`${integration.config.baseUrl}/rest/api/3/myself`, {
        headers: { authorization: this.authHeader(creds, integration.config.email), accept: "application/json" },
      })
      if (!res.ok) return { ok: false, detail: `jira /myself: HTTP ${res.status}` }
      const body = (await res.json()) as { displayName?: string }
      return { ok: true, detail: `authenticated${body.displayName ? ` as ${body.displayName}` : ""}` }
    } catch (err) {
      return { ok: false, detail: (err as Error).message }
    }
  }

  async poll(integration: Integration, creds: CredentialsInput, cursor: string | undefined): Promise<PollResult> {
    ...
    const params = new URLSearchParams({ jql: clause, maxResults: "50", fields: "summary,updated,reporter,description" })
    ...
    if (res.status === 429) throw new Error(`jira rate limited (retry_after ${res.headers.get("retry-after") ?? "?"})`)
    const body = (await res.json()) as SearchResponse
    if (!res.ok) throw new Error(`jira search: ${body.errorMessages?.join("; ") ?? `HTTP ${res.status}`}`)
  ...
  async send(integration: Integration, creds: CredentialsInput, item: ChannelItem, text: string): Promise<void> {
    ...
    const res = await this.fetchImpl(`${integration.config.baseUrl}/rest/api/3/issue/${key}/comment`, {
      method: "POST",
      headers: {
