...
describe("CalendarChannelAdapter", () => {
  ...
  it("skips cancelled events and passes the cursor as updatedMin", async () => {
    ...
    const eventsUrl = (fetchImpl as unknown as ReturnType<typeof vi.fn>).mock
      .calls[1]![0] as string;
  ...
  it("drains nextPageToken pages so the cursor never skips an un-read tail", async () => {
    const fetchImpl = sequenceFetch(
      { body: { access_token: "ya29.token" } },
      {
        body: {
          nextPageToken: "page2",
          items: [
            { id: "evt1", status: "confirmed", updated: "2026-06-17T08:00:00.000Z", summary: "A" },
          ],
        },
      },
      {
        body: {
          items: [
            { id: "evt2", status: "confirmed", updated: "2026-06-17T12:00:00.000Z", summary: "B" },
          ],
        },
      },
    );
    ...
    const secondPageUrl = (fetchImpl as unknown as ReturnType<typeof vi.fn>).mock
      .calls[2]![0] as string;
  ...
  it("test maps the calendar lookup to a TestResult", async () => {
    ...
    const adapter = new CalendarChannelAdapter(fetchImpl);
    expect(await adapter.test(cal, creds)).toEqual({
      ok: true,
      detail: "connected to Karel's calendar",
    });
  });
