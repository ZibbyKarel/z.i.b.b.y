...
describe("GitHubChannelAdapter", () => {
  it("normalizes issues + PRs with distinct ids and advances the cursor", async () => {
    const fetchImpl = jsonFetch([
      { number: 1, title: "Crash on login", body: "stack trace", updated_at: "2026-06-17T09:00:00.000Z", user: { login: "dana" } },
      { number: 2, title: "Add caching", updated_at: "2026-06-17T10:00:00.000Z", user: { login: "eli" }, pull_request: { url: "x" } },
    ])
  ...
  it("respects the streams filter (issues only drops PRs)", async () => {
    const issuesOnly: Integration = { ...gh, config: { kind: "github", repo: "acme/app", streams: ["issues"] } }
    const fetchImpl = jsonFetch([
      { number: 1, title: "issue", updated_at: "2026-06-17T09:00:00.000Z", user: { login: "dana" } },
      { number: 2, title: "pr", updated_at: "2026-06-17T10:00:00.000Z", user: { login: "eli" }, pull_request: {} },
    ])
    const adapter = new GitHubChannelAdapter(fetchImpl)
  ...
  it("test maps /user to a TestResult", async () => {
    ...
    expect(await ok.test(gh, { token: "ghp" })).toEqual({ ok: true, detail: "authenticated as dana" })
  })
  ...
  it("send posts a comment to the issue/PR", async () => {
    ...
    const adapter = new GitHubChannelAdapter(fetchImpl)
    const item = {
      id: "gh-acme-app-issue-1", integrationId: "acme-gh", kind: "github" as const,
      externalRef: { channel: "acme/app", messageId: "1" }, receivedAt: "2026-06-17T00:00:00.000Z",
