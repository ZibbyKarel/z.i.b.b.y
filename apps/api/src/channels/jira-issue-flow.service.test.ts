...
function build(over: { integration?: Integration | null; creds?: unknown } = {}) {
  ...
  const credentials = {
    read: vi.fn(async () => (over.creds === undefined ? { token: "tok" } : over.creds)),
  };
...
describe("JiraIssueFlowService", () => {
  ...
  it("rejects a non-jira integration", async () => {
    ...
    await expect(svc.propose({ integrationId: "acme-jira", summary: "x" })).rejects.toThrow(
      /not a jira integration/,
    );
  });
  ...
  it("resume creates the issue exactly once and records the approval", async () => {
    ...
    expect(activity.record).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        refs: expect.objectContaining({ status: "BUG-9" }),
      }),
    );
