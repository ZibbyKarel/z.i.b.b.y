...
describe("ActivityLogService", () => {
  ...
  it("rotates files on the day boundary (date from injected now)", async () => {
    await service.record({ kind: "run-started", summary: "late" }, new Date("2026-06-12T23:59:59.900Z"))
    await service.record({ kind: "run-finished", summary: "early" }, new Date("2026-06-13T00:00:00.100Z"))
  ...
  it("readSince spans yesterday + today and filters by timestamp", async () => {
    ...
    await service.record({ kind: "task-created", summary: "yesterday-early" }, new Date("2026-06-12T06:00:00.000Z"))
    await service.record({ kind: "task-created", summary: "yesterday-late" }, new Date("2026-06-12T20:00:00.000Z"))
    await service.record({ kind: "task-created", summary: "today" }, new Date("2026-06-13T07:00:00.000Z"))
