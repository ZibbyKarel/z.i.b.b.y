 ...
 *
 * NS2 F3a — no write-time harden-only 422 for department-tagged rules HERE, by
 * decision: the evaluator now lives downstream of this module (`GatesModule`
 * imports `GateRulesModule` to feed the per-department bucket), so injecting
 ...
 * would only have been a write-time UX nicety (`validateDepartmentRuleHardenOnly`
