...
describe("formatClaudeStreamLine", () => {
  ...
  it("renders an assistant text block as its text", () => {
    const out = formatClaudeStreamLine(
      line({ type: "assistant", message: { content: [{ type: "text", text: "Scanning /tmp/x" }] } }),
    )
  ...
  it("renders a file tool call with its path", () => {
    const out = formatClaudeStreamLine(
      line({
        type: "assistant",
        message: { content: [{ type: "tool_use", name: "Read", input: { file_path: "/tmp/a.txt" } }] },
      }),
    )
  ...
  it("renders the system init line with the model", () => {
    const out = formatClaudeStreamLine(line({ type: "system", subtype: "init", model: "claude-haiku" }))
