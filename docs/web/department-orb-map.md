# Department orb map (`features/chat/components/DepartmentOrbMap.tsx`)

The ambient backdrop of ZIBBY's chat surface (the Velín-D "velín"): a central
conversational core orb ringed by one orb per department, laid out on an
...
`DepartmentOrbMap` itself is a thin domain→DS adapter: it owns no layout math.
...
component's job is mapping ZIBBY's department registry, live runs and the
...
reserve without overlap. `DepartmentOrbMap` always renders the fixed 8-entry
`DEPARTMENTS` registry in registry order, so the ring never reflows when the
feed order changes.
...
| Prop                            | Purpose                                                                |
| ------------------------------- | ---------------------------------------------------------------------- |
| `departments`                   | `DepartmentWithStatus[]` — the live department roster + state          |
| `runs` / `pipelines` / `agents` | Feed `activeRunsByDepartment` for each node's orbit-field dot count    |
...
| `onSelectDepartment`            | Fires with a department id when its orb is selected (opens the drawer) |
...
Each `DEPARTMENTS` entry becomes an `OrbMapNode`: its status state maps through
`DEPARTMENT_ORB_STATE`, its glyph through `DEPARTMENT_GLYPH`, and its active-run
count comes from `activeRunsByDepartment(runs, pipelines, agents)`. A run
attributes to a department via its owning pipeline's `department` (a
`pipeline`-kind run) OR its owning agent's `department` (an `agent`-kind
...
run never attributes (D16 — no `department` concept exists on the goal
schemas). The core's
...
`DepartmentOrbMap` owns the comet handoff-flares' state end to end — the gap
...
gate to both) that resolves to an owning department becomes a flare from the
core to that department (`running`, a dispatch) or from the department back to
...
(D16 — no `department` concept exists on the goal schemas).
...
department (or the query settling) resolves normally. Accepted as-is, same
posture as the run-cache race already documented on `resolveEventOwner` — no
handling added for it.

## Colours

Each department's identity colour comes straight from the `DEPARTMENTS`
...
`DepartmentOrbMap` full-page, behind every interactive surface (its DOM layers
