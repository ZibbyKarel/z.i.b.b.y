...
`apps/web/features/chat/**`, `apps/web/features/departments/components/DepartmentDrawer/**`.
...
| gates (route-only) | `/gates`                                                 | částečně — `GatesTab` = gate rules per department, odkazuje jen na `/projects/:id?tab=profile` | ne — samotné `/gates` (globální policy floor) nikde |                                                                                                                                                                                   |
...
mají subsystémy (`DepartmentDrawer`).
...
- **`/gates`** globální panel — `GatesTab` řeší jen per-department/per-project pravidla,
