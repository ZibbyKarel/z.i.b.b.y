...
- [ ] **T3 — typecheck**: implement the `open-url` case in `machine.service.ts` `plan()` and `execute()` (http(s)-only fail-closed guard per the schema's documented behavior; reuse the `opener`), plus an exhaustiveness `never` check so a future 5th kind fails at compile time. Verify `pnpm check:types` passes.
