...
| `maxConcurrentRoadmapRuns` | `3`      | Ceiling on roadmap items `running` at once, across every project — one rule for manual play and the auto-pickup tick alike. Separate from `maxConcurrentRuns` so a big epic can't starve ad-hoc tasks. Counts `running` only; `awaiting-merge` frees its slot                     |
