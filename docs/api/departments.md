...
| Contract     | `libs/contracts/src/departments/departments.contract.ts`                   | `departmentsContract` — `getDepartments` (`GET /api/departments`), `getDepartment` (`GET /api/departments/:id`, 404 on unknown id), `markDepartmentSeen` (`POST /api/departments/:id/seen`, 404 on unknown id)                          |
