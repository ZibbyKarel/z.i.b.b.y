...
| `SpeechModule`            | `speech/speech.module`                 | Thin HTTP proxy to the local `speakd` TTS daemon                                                                                                                                                                         |
| `TasksModule`             | `tasks/tasks.module`                   | Task creation, scheduling, and the unified `/api/tasks/runs/*` run surface                                                                                                                                               |
| `AttachmentSetRefsModule` | `tasks/attachment-set-refs.module`     | Phase 116b: no-cycle contributor registry for `TaskSchedulerService`'s 24h attachment orphan sweep — the seam that lets it read `AutomationsStorageService` without `TasksModule` importing `AutomationsModule` directly |
