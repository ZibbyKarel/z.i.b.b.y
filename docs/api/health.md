...
process liveness. `DepartmentHealthService` (`apps/api/src/health/department-health.service.ts`)
probes every department the operator depends on; `HealthController`
...
| Piece      | File                                               | Role                                                                                                                                      |
| ---------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Contract   | `libs/contracts/src/health/health.schema.ts`       | `HealthSchema`, `DepartmentHealthSchema` (`name`/`status`/`detail`), `ClaudeHealthSchema`                                                 |
...
| Service    | `apps/api/src/health/department-health.service.ts` | `DepartmentHealthService.probeAll()` — probes vault, integrations, scheduler concurrently                                                 |
...
2. Each probe is defensive by construction: it can only resolve to a
   `DepartmentHealth` row, never throw, so one unreachable department degrades only
...
3. `HealthController.getHealth` runs the Claude CLI preflight
   ...
   department is not `ok`; `"ok"` otherwise. The payload also carries `uptime`
   (`process.uptime()`) and an ISO `timestamp`.

This is the M8 "never fail silently" floor: the dashboard never has to infer a fault
from an absence of data — a broken department shows up as its own named, `down` or
`degraded` row in the same response the liveness check already returns.

## Endpoints (`/api/health`)

- `GET /health` — liveness + readiness in one payload:
  `{ status: "ok" | "degraded", uptime, timestamp, claude, departments }`, where
  `claude` is `{ ok, version?, reason? }` from the CLI preflight and `departments`
