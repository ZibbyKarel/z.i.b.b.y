...
40 domain folders exist today under `libs/contracts/src/` (every one below,
...
| `roadmap/`         | `RoadmapItemSchema`, `LevelMappingSchema`, `RoutingProposalSchema`                                                               | Per-project delivery roadmap (Phase 125) + the global level-mapping table + NS2 F10's parked Tier-3 routing question      |
| `tasks/`           | `ScheduledTaskSchema`, `TaskRoutingSchema`, `RoutingAlternativeSchema`, `TaskRunSchema` (`task-run.schema.ts`)                   | Deferred tasks + the unified run surface (`task-runs.contract.ts`)                                                        |
...
| `review-learning/` | `ReviewRuleSchema`, `ReviewRuleOccurrenceSchema`, `ReviewRulesFileSchema`                                                        | Rules learned from PR code-review comments (see `docs/api/review-learning.md`)                                            |
| `registries/`      | `RegistryBindingsSchema`                                                                                                         | ZB-11 — derived "Bound in" department data for the skills/mcp/hooks/commands registries (see `docs/api/extensibility.md`) |
